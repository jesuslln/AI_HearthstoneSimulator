...
class BaseGame(Entity):
	...
	def action_end(self, type, source):
		...
		if self.ended:
			if str(self.player1.playstate) == "PlayState.WON":
				raise GameOver("Player1")
			elif str(self.player2.playstate) == "PlayState.WON":
				raise GameOver("Player2")
			else:
				raise GameOver("The game has ended as a Tie.")
